...
public class PrintFileTree {
	 ...
	 */
	public PrintFileTree(int sorting) {
		howSort = sorting;
	}
	 ...
	 * represented by r
	 */
	public String printTree(TreeNode<FileObject> r, int indent) {
		String list = "";
		String spaces = "";
		if (indent == 0) {
			longestName = 0;
		...
		if (r.getData().isDirectory()) {
			...
			// from subdirectories
			Iterator<TreeNode<FileObject>> dir;
			...
			if (dir == null)
				return list;
			while (dir.hasNext()) {
				TreeNode<FileObject> child = dir.next();
