...
public class TestFileTree {
	...
	public static void main(String[] args) {
		...
		// Test getChildren method. Children must be retrieved in the proper
		// order
		...
		// Test getChildren(Comparator). Children must be retrieved in the
		// proper order
		...
		FileObject.setTestingMode();
		TreeNode<FileObject> r;
		testPassed = true;
		try {
			FileTree tree = new FileTree("folder1");
	...
	private static void test7() {
		...
		try {
			...
			if (FileObject.testFailed())
				...
				if (!s.equals("folder3/folderC/folderE/file6.java"))
					testPassed = false;
	...
	// Test 9. Bonus marks. Test duplicatedFiles and duplicated folders
	private static void test9_10(int test) {
		String[] output9 = { "folder3/folderA/folderB/file1.gif", "folder3/folderB/file1.gif",
				"folder3/folderA/folderD/file2.java", "folder3/folderC/folderE/folderD/file2.java",
				"folder3/folderA/folderD/folderF/file3.java", "folder3/folderC/folderE/folderD/folderF/file3.java",
				"folder3/folderA/folderD/folderF/file4.txt", "folder3/folderC/folderE/folderD/folderF/file4.txt",
				"folder3/folderA/folderD/folderF/file5.gif", "folder3/folderC/folderE/folderD/folderF/file5.gif" };
		String[] output10 = { "folder3/folderA/folderB", "folder3/folderB", "folder3/folderA/folderD",
				"folder3/folderC/folderE/folderD", "folder3/folderA/folderD/folderF",
				"folder3/folderC/folderE/folderD/folderF" };
		...
		try {
			tree = new FileTree("folder3");
			if (FileObject.testFailed())
				System.out.println("Test " + test + " failed: Inexistent file object name");
			else {
				if (test == 9)
					list = tree.duplicatedFiles();
				else
					list = tree.duplicatedFolders();

				// Check that files/folders with the same name appear in
				// consecutive positions of the iterator
				while (list.hasNext()) {
					s = list.next();
					s1 = list.next();
					if (!getShortName(s).equals(getShortName(s1))) {
						testPassed = false;
						break;
					}
				}

				// Check that all duplicated files/folders were found
				if (test == 9)
					list = tree.duplicatedFiles();
				else
					list = tree.duplicatedFolders();
				...
				while (list.hasNext()) {
					s = list.next();
					// Look for s in output
					for (int i = 0; i < output.length; ++i)
						if (output[i].equals(s)) {
							output[i] = "";
							break;
						}
				}

				// Verify all entries in output were marked
				int index = 0;
				...
				if (index < output.length || list.hasNext())
					testPassed = false;
				// Check that all duplicated files were found
